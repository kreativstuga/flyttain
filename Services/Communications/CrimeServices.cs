...
namespace FlyttaIn.Services.Communications
{
    public class CrimeServices
    {
        ...
        public List<CrimeEvent> GetCrimes(int minutesPassed, string lan)
        {
            List<CrimeEvent> retval = new List<CrimeEvent>();
            var newUrl = string.Format(CRIMEURL, minutesPassed.ToString(), lan);
            ...
            {
                var eventChildren = eventObject.Cast<XmlNode>();
